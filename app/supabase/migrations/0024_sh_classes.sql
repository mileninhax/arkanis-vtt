...
select (select id from classes where slug = 'combatente'), v.name, v.description, v.prereq, v.ord from (values
  ...
  ('Caminho para Forca', 'Ação sacrifício (cena de perseguição), gasta 1 PE pra +◯ extra (total +2◯) nos testes dos outros; ação chamar atenção (cena de furtividade), gasta 1 PE pra reduzir visibilidade dos aliados em -2 (em vez de -1).', null, 21),
  ...
  ('Correria Desesperada', '+3m deslocamento; +◯ em perícia pra fugir em perseguição.', null, 23),
...
select (select id from class_tracks where slug = 'cacador' and class_id = (select id from classes where slug = 'combatente')), v.nex, v.name, v.description from (values
  ...
  (65, 'Atacar das Sombras', 'Sem penalidade -◯ em Furtividade por se mover no deslocamento normal; atacar com arma silenciosa na mesma rodada reduz a penalidade pra -◯; visibilidade inicial em furtividade sempre 1 ponto abaixo (pode ficar negativa).'),
  (99, 'Estudar a Presa', 'Usando Estudar Fraquezas contra criatura/cultista, pode marcá-lo como "presa" — contra o tipo, +◯ em testes de perícia, +1 margem de ameaça e multiplicador de crítico, resistência a dano 5. Só uma presa por vez.')
) as v(nex, name, description);
...
select (select id from class_tracks where slug = 'monstruoso' and class_id = (select id from classes where slug = 'combatente')), v.nex, v.name, v.description from (values
  ...
  (40, 'Ser Macabro', 'Resistência da etapa ritualística sobe pra 10, penalidade em perícias sobe pra -2◯; ganha efeitos adicionais por elemento (ver descrição completa no documento de conteúdo).'),
...
select (select id from classes where slug = 'especialista'), v.name, v.description, v.prereq, v.ord from (values
  ...
  ('Esconderijo Desesperado', 'Sem penalidade -◯ em Furtividade ao mover no deslocamento normal; em cena de furtividade, sucesso ao esconder reduz visibilidade em -2 (em vez de -1).', null, 19),
  ...
  ('Mãos Firmes', 'Teste de Furtividade pra esconder ou ação discreta manipulando objeto, gasta 2 PE pra +◯.', 'Treinado em Furtividade', 23),
...
select (select id from class_tracks where slug = 'perseverante' and class_id = (select id from classes where slug = 'especialista')), v.nex, v.name, v.description from (values
  ...
  (40, 'Fuga Obstinada', '+◯ em testes de perícia pra fugir de inimigo (perseguição ou não); sendo a presa numa perseguição, acumula até 4 falhas antes de ser pego (em vez do padrão).'),
...
select (select id from classes where slug = 'ocultista'), v.name, v.description, v.prereq, v.ord from (values
  ...
  ('Sincronia Paranormal', 'Ação padrão + 2 PE estabelece sincronia mental com personagens em alcance médio que já sobreviveram a um encontro paranormal com você; no início de cada rodada, distribui ◯ = Presença entre os participantes (usável em testes Int/Pre, some no fim da rodada); manter custa 1 PE/rodada.', 'Pre 2', 24),
...
select (select id from class_tracks where slug = 'exorcista' and class_id = (select id from classes where slug = 'ocultista')), v.nex, v.name, v.description from (values
  ...
  (40, 'Poder da Fé', 'Vira veterano (ou +◯ se já veterano) em Religião; falha em teste de resistência, gasta 2 PE pra rerrolar usando Religião (aceita o novo resultado).'),
...
select (select id from class_tracks where slug = 'possuido' and class_id = (select id from classes where slug = 'ocultista')), v.nex, v.name, v.description from (values
  ...
  (40, 'As Sombras Dentro de Mim', 'Recuperação de PP sobe pra 2 por dormir. Gasta 2 PE pra deixar a Entidade controlar os músculos por 1 rodada: +◯ em Acrobacia/Atletismo/Furtividade, e em cena de furtividade o aumento de visibilidade por qualquer ação nessa rodada é reduzido em -1.'),
...
select (select id from classes where slug = 'sobrevivente'), v.name, v.description, v.ord, true from (values
  ...
  ('Cicatrizado (Estágio 5)', 'Escolhe um tipo de perigo paranormal de elemento específico já enfrentado — sofre -◯ em resistência contra ele, mas 1x/sessão pode, como reação, sacrificar 1 PV permanente pra ignorar dano mental/gasto de PE, ou sacrificar 1 PE permanente pra reduzir dano físico à metade.', 3)
...
select (select id from class_tracks where slug = 'durao' and class_id = (select id from classes where slug = 'sobrevivente')), v.estagio, v.name, v.description from (values
  ...
  (4, 'Pancada Forte (Estágio 4)', 'Ataque, gasta 1 PE pra +◯ no teste (se virar combatente, perde essa habilidade mas reduz custo de Ataque Especial em -1 PE).')
